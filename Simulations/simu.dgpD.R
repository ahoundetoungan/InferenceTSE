...
fsim   <- function(N, S = 2, k = 1e3){#S is the number of returns and k is kappa in the paper
  ...
  tryCatch(
    expr = {
      ...
      En      <- sapply(1:k, function(x) sum(d1lclaytonpdf(u = pzs[[x]], ltheta = lths1, dim = S)))/sqrt(N)
      Omega   <- mean(En)
      Vh      <- fHAC(d1lclaytonpdf(u = pzh, ltheta = lths1, dim = S), N, r = 6*S)
      psis1   <- sapply(1:k, function(x) (sqrt(Vh)*rnorm(1) + En[x] - Omega)/An)
      
      # # Inference: mean correction
      An      <- -sum(d2lclaytonpdf(u = pzh, ltheta = lths2, dim = S))/N
      En      <- sapply(1:k, function(x) sum(d1lclaytonpdf(u = pzs[[x]], ltheta = lths2, dim = S)))/sqrt(N)
      Omega   <- median(En)
      Vh      <- fHAC(d1lclaytonpdf(u = pzh, ltheta = lths2, dim = S), N, r = 6*S)
      psis2   <- sapply(1:k, function(x) (sqrt(Vh)*rnorm(1) + En[x] - Omega)/An)
...
fdata    <- function(s){
      ...
      H2    <- rowMeans(do.call(cbind, mclapply(datas, function(x) pnorm(t, 0, x$sderr), mc.cores = 11L)), na.rm = TRUE)
      ...
      data.frame(s    = s,
        ...
        mutate(type   = factor(type, labels = expression(paste(sqrt(n)*(log(hat(theta)[n]) - log(theta[0])), 
                                                               ", ", n, " = ", !!Nsl, ", ", k[n], " = ", !!Ss))),
               typec   = factor(type, labels = expression(paste(sqrt(n)*(log(hat(theta)[n]^"*") - log(theta[0])), 
                                                                ", ", n, " = ", !!Nsl, ", ", k[n], " = ", !!Ss)))) 
}
